...
export default function TreatmentHistory({ navigation }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const createdGoals = await API.graphql<
        GraphQLQuery<UpdateUserGoalsMutation>
      >(
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={{ flex: 1 }}>
          ...
          <Text style={styles.question}>
            {" "}
            What treatments have you been taking?{" "}
          </Text>
          ...
          <View style={styles.datePickersContainer}>
            ...
            >
              <View style={styles.datePicker}>
                <Text style={styles.dateText}>
                  To: {toDate ? toDate.toLocaleDateString() : "Select a date"}
                </Text>
                <Ionicons
                  name="calendar"
                  size={30}
              ...
              <ScrollView>
                {treatments.map((treatment, index) => (
                  <View key={index} style={styles.treatmentItem}>
                    <Text style={{ flex: 1 }}>
                      {treatment.name} (From:{" "}
                      {new Date(treatment.from).toLocaleDateString()} To:{" "}
                      {new Date(treatment.to).toLocaleDateString()})
                    </Text>
                    <TouchableOpacity
